...
const ViewIndividualPlan = () => {
    ...
    const getIndividualPlans = async () => {
        try {
            const { data } = await axios.get(`${API_URL}/api/indivual_plan_master`);
    ...
    const handleGetUsers = async () => {
        try {
            const { data } = await axios.get(`${API_URL}/api/indivual_plan_master/user`);
    ...
    const handleDelete = async (id) => {
        swal({
            ...
            if (willDelete) {
                try {
                    await axios.delete(`${API_URL}/api/indivual_plan_master/delete/${id}`);
                    getIndividualPlans(); // Refresh the data after deletion
                } catch (error) {
                    console.error('Error message:', error.message);
                }
            }
    ...
    let paggination = Array(Math.ceil(userData && userData.length / sort)).fill().map((_, i) => i + 1);
    console.log("userData", userData);
    const handleDateFilter = async () => {
        setShow2(false)
        try {
            const { data } = await axios.post(`${API_URL}/api/indivual_plan_master/datefilter`, {
                "startDate": date,
                "endDate": endDate
            })
    ...
    const handleUserChange = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post(`${API_URL}/api/indivual_plan_master/datefilter`, para)
    ...
    return (
        <Fragment>
            <AdminPageTitle activePage="Manage orders" pageName="Individual Plans" />
                ...
                <Card>
                    <div className="row">
                      ...
                      <input
                        ...
                        />
                        <div  style={{ width: "fit-content", marginLeft: "25px", marginTop: "10px" }}>
                            <ExportToExcel apiData={userData} fileName={"Individual-plans"} />
                        </div>
                    </div>
                    <Card.Body>
                                    ...
                                    <tr key={i}>
                                        ...
                                        <td>
                                            <div className="d-flex">
                                                <Link
                                                    to={`/admin/view/edit_individual_plan/${user.id}`}
...
export default ViewIndividualPlan;
