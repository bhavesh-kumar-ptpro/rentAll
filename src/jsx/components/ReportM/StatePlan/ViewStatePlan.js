...
const ViewStatePlans = () => {
    ...
    const getIndividualPlans = async () => {
        try {
            const { data } = await axios.get(`${API_URL}/api/state_plan_master`);
    ...
    const handleGetUsers = async () => {
        try {
            const { data } = await axios.get(`${API_URL}/api/state_plan_master/user`);
    ...
    const handleDateFilter = async () => {
        setShow2(false)
        try {
            const { data } = await axios.post(`${API_URL}/api/state_plan_master/datefilter`, {
                "startDate": date,
                "endDate": endDate
            })
    ...
    const handleUserChange = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post(`${API_URL}/api/state_plan_master/datefilter`, para)
    ...
    return (
        <Fragment>
            <AdminPageTitle activePage="Manage orders" pageName="State Plans" />
                    ...
                    {users &&    <select
                      ...
                      <input
                        ...
                        <div  style={{ width: "fit-content", marginLeft: "25px", marginTop: "10px" }}>
                            <ExportToExcel apiData={userData} fileName={"corporate-excel"} />
                        </div>
                    </div>
                    <Card.Body>
