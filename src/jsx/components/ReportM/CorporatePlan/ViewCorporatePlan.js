...
const ViewCorporatePlan = () => {
    ...
    let activeStyle = { background: "#00a65a" }
    useEffect(() => {
        handleGetHMO();
        handleGetUsers();
    }, []);
    ...
    const handleGetHMO = async () => {
        try {
            const { data } = await axios.get(`${API_URL}/api/corporate_plan_master`);
            setUserData(data);
        } catch (error) {
    ...
    const handleGetUsers = async () => {
        try {
            const { data } = await axios.get(`${API_URL}/api/corporate_plan_master/user`);
    ...
    const handleDelete = async (id) => {
        swal({
            ...
            if (willDelete) {
                try {
                    await axios.delete(`${API_URL}/api/corporate_plan_master/delete/${id}`);
    ...
    const onClick = (i) => {
        ...
        activePag.current = i + 1;
        console.log("onClick", activePag.current);
    };
        ...
        const searchTerm = e.target?.value?.toLowerCase() ?? ''; if (searchTerm !== "") {
            try {
                const filteredData = userData.filter(item => {
                    ...
                    return regex.test(item.agency_name) || regex.test(item.email);
    ...
    const handleDateFilter = async () => {
        setShow2(false)
        try {
            const { data } = await axios.post(`${API_URL}/api/corporate_plan_master/datefilter`, {
                "startDate": date,
                "endDate": endDate
            })
    ...
    const handleUserChange = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post(`${API_URL}/api/corporate_plan_master/datefilter`, para)
    ...
    return (
        <Fragment>
            <AdminPageTitle activePage="Manage orders" pageName="Corporate Plans" />
            ...
            <Col lg={12}>
                <Card>
                    <div className="row">
                        ...
                        />
                        <div style={{ width: "20%", marginTop: "10px", marginLeft: "5%" }}>
                            <ExportToExcel apiData={userData} fileName={"corporte-plans"} />
                        </div>
                    </div>
                    <Card.Body>
                            ...
                            <tbody>
                                {userData.length > 0 ? userData.slice(activePag.current * sort, (activePag.current + 1) * sort).map((user, i) => (
                                    <tr key={i}>
                                        ...
                                        <td>{user.no_of_staff}</td>
                                        <td>{user.budget}</td>

                                        <td>{user.state}</td>
                                        <td>{user.local_govt}</td>
                                        <td>{moment(user.CreateDTTM).format("yyyy-mm-DD HH:mm:ss")}</td>
                                        <td>
                                    ...
                                    <span>
                                        {paggination.map((number, i) => (
                                            <Link
                                                key={i}
                                                to="/admin/view/view_corporate_plan"
                                                className={`paginate_button ${activePag.current == i + 1 ? "current" : ""}`}
                                                onClick={() => onClick(i)}
                                            // style={activePag.current == number ? activeStyle:""}
                                            >
                                                {number}
                                            </Link>
                                        ))}
...
export default ViewCorporatePlan;
